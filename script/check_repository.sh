...
# Immutable release identity. Including BUILD_NUMBER in the tag means either
# VERSION or BUILD_NUMBER can advance without moving an already-published tag.
...
if git rev-parse -q --verify "refs/tags/${tag}" >/dev/null 2>&1; then
  ...
  if [[ -n "$tag_sha" && -n "$head_sha" && "$tag_sha" != "$head_sha" ]]; then
    ...
    if [[ "${BARTENDER_ENFORCE_RELEASE_TAG:-}" == "1" ]]; then
      fail "release tag ${tag} already exists for ${tag_sha}; bump VERSION or BUILD_NUMBER before publishing (HEAD is ${head_sha})"
    fi
  fi
fi
